...
<div class="row">
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="page-header">
            <h2 class="pageheader-title">Danh sách tin tức
            </h2>
                        ...
                        <tbody>
                            ...
                            @foreach ($dataBlog as $element)
                                <tr class="tr-item" id="tr-item-{{ $element->id}}">
                                    <td>{{$stt++}}</td>
                                    <td>{{$element->name}}</td>
                                    <td><img src="{{ asset('public/uploads/images/blog/'.$element->image) }}" height="200"></td>
                                    <td>{!!$element->description!!}</td>
                                    <td>
                                        @can('edit_blog')
                                        <button class="btn btn-primary" onclick="window.location.href='{{ url('admin/blog/edit-blog/'.$element->id) }}'" data-toggle="tooltip" data-placement="top" title="Sửa tin tức">Sửa</button>
                                        @endcan
                                        @can('delete_blog')
                                        <button class="btn btn-danger btn-del" data-id="{{ $element->id }}" data-toggle="tooltip" data-placement="top" title="Xóa tin tức">Xóa</button>
                                        @endcan
                                    </td>                                    
                                </tr>
                            @endforeach
    ...
    $(document).on('click', '.btn-del', function () {
        ...
        Swal({
            ...
            if (result.value == true) {
                $.ajax({
                    url: '{{ url('admin/blog/delete-blog') }}',
                    type: 'POST',
                    data: {
                        id: id,
                        length: '1'
                    ...
                    success: function (data) {
                        ...
                        if (data.status == '_success') {
                            Swal({
                                ...
                                $("#tr-item-" + id).remove();
                                if ($("#blog-table .tr-item").length == 0) {
                                    location.reload();
                                }
                            });
                        } else {
